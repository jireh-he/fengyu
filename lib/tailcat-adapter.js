 ...
 *   3. PATH 里的 tailcat
 *
 * Node 16 兼容（低版本系统环境）：只用 core modules，无 native 依赖。
 */
...
function binaryHelp() {
  ...
  console.log('[fengyu] 未找到 tailcat 二进制。安装方式（三选一）：');
...
function assertAddr(addr, action) {
  if (!ADDR_PREFIX_RE.test(String(addr || ''))) {
    console.log('[fengyu] "' + String(addr) + '" 不像 P2P 地址（应以 tc 开头），' + action + ' 无法执行。');
