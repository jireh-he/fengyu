#!/usr/bin/env sh
# fengyu-p2p 一键安装 — 让任何终端/AI agent 获得 P2P 聊天 & 文件传输能力
#
# 用法：
#   sh skill/fengyu-p2p/install.sh           # 装到当前 fengyu 仓库根目录（开发模式）
#   FENGYU_HOME=$HOME/.fengyu sh skill/fengyu-p2p/install.sh   # 独立安装到 ~/.fengyu
#
# 依赖：git + node(>=16)。P2P 路径零 npm 依赖（静态 tailcat 二进制 + Node 内置模块）。
set -eu

REPO_URL="${FENGYU_REPO:-https://github.com/jireh-he/fengyu-p2p.git}"
...
if [ "$in_repo" = "1" ]; then
  ...
  if [ ! -d "$ROOT/.git" ]; then
    echo "[fengyu-p2p] cloning $REPO_URL → $ROOT"
